            ...
            <div v-for="(row, ri) in templateRows" :key="ri" class="flex gap-[20px] w-full">
              ...
              >
                <div class="flex items-center justify-center h-[140px]" :style="{ background: tpl.gradient }">
                  <span class="material-symbols-outlined text-[40px] text-white/40">{{ tpl.icon }}</span>
                </div>
                <div class="flex flex-col gap-[6px] p-[16px_20px]">
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-[4px]">
                      <span class="material-symbols-outlined text-[14px] text-[var(--accent-secondary)]">star</span>
...
const templates = [
  { id: 1, name: 'Dashboard Pro', category: 'dashboard', description: 'Full-featured admin dashboard with analytics', users: 2340, rating: 4.8, icon: 'monitoring', gradient: 'linear-gradient(135deg, #1E1E2E, #2D1B4E)' },
  { id: 2, name: 'Shop Starter', category: 'ecommerce', description: 'E-commerce storefront with cart & checkout', users: 1890, rating: 4.7, icon: 'storefront', gradient: 'linear-gradient(135deg, #0D2137, #1A3A5C)' },
  { id: 3, name: 'Blog Kit', category: 'blog', description: 'Content-first blog with CMS integration', users: 1560, rating: 4.6, icon: 'article', gradient: 'linear-gradient(135deg, #1A2E1A, #2D4A2A)' },
  { id: 4, name: 'Portfolio Plus', category: 'portfolio', description: 'Showcase portfolio with stunning effects', users: 980, rating: 4.9, icon: 'palette', gradient: 'linear-gradient(135deg, #3E2723, #5D4037)' },
  { id: 5, name: 'Landing Page Pro', category: 'landing', description: 'Conversion-optimized landing pages', users: 3200, rating: 4.8, icon: 'rocket_launch', gradient: 'linear-gradient(135deg, #1A1A2E, #2D1B5E)' },
  { id: 6, name: 'Admin Pro', category: 'admin', description: 'Complete admin panel with user management', users: 1450, rating: 4.5, icon: 'admin_panel_settings', gradient: 'linear-gradient(135deg, #2E1A1A, #5E2D1B)' },
]
...
function useTemplate(tpl) {
  router.push({ path: '/editor', query: { template: tpl.id } })
}
</script>
